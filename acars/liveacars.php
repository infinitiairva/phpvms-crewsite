...
Class acarsPosition
{
    ...
    function doInsert()
    {
        ...
        if( $this->msgtype <> 'ZZ' )
        {
            ...
            $newID = mysqli_insert_id($this->conn); // maybe this function does not work on each system
            $query = "UPDATE acars_flight SET curPositionID = {$newID}, active = 1 WHERE acarsFlightID = {$this->flightID};";
            $result = mysqli_query($this->conn, $query) or die("0|SQL query failed (UPDATE acars_flight)");
        }
...
{
    global $conn;
    $q_user = @mysqli_query($conn, "SELECT id FROM user WHERE isactive=1 and username = '{$pid}' LIMIT 1");
...
switch ($data2) {
    ...
    case "BEGINFLIGHT":
        ...
        $acarspos = new acarsPosition($conn);
    ...
    case "MESSAGE":
        ...
        switch( $acarspos->msgtype )
        {
            ...
            case 'QB':   // QB = OFF Message
                ...
                $result = @mysqli_query($conn, $query) or die("0|SQL query failed");
